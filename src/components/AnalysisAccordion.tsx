              ...
              {data.map((item, index) => (
                <tr key={index}>
                  {keys.map((key) => {
                    ...
                    return (
                      <td key={key} className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 break-words">
                        {typeof value === 'object' && value !== null ? (
                          ...
                          <div className="break-words" title={String(value)}>
                            {(() => {
                              ...
                              return <span className="break-words">{urlString}</span>;
                            })()}
                          </div>
                        )}
                      </td>
  ...
  const parseJsonFromMarkdown = (text: string): unknown => {
    ...
    if (jsonMatch) {
      try {
        ...
        if (jsonText.includes('"industries_using_technology"')) {
          ...
          if (startMatch) {
            ...
            if (objects.length > 1) {
              ...
              jsonText = jsonText.substring(0, startMatch.index) + 
                        replacement + 
        ...
        if (jsonText.includes('"industries_using_technology"') && !jsonText.includes('"industries_using_technology": [')) {
          // Look for the pattern: "industries_using_technology": { ... }, { ... }, { ... }
          const pattern = /"industries_using_technology":\s*\{[^}]*\}(?:\s*,\s*\{[^}]*\})+/;
          ...
          if (match) {
            ...
            if (objectMatches && objectMatches.length > 1) {
              ...
              jsonText = jsonText.substring(0, startIndex) + 
                        replacement + 
            ...
            for (let i = startIndex; i < jsonText.length; i++) {
              ...
              if (!inString) {
                if (char === '{') {
                  ...
                  if (remaining.startsWith('"') && !remaining.startsWith('"industries_using_technology')) {
                    endIndex = i;
                    break;
                  }
                }
    ...
    if (typeof content === "string") {
      ...
      if (parsedJson) {
        ...
        if (category === 'founder_profile') {
          return renderTableFromObject(parsedJson as Record<string, unknown>, 'Founder Information');
        } else if (category === 'industry_analysis') {
          return renderTableFromObject(parsedJson as Record<string, unknown>, 'Industry Analysis');
        ...
        return renderTableFromObject(parsedJson as Record<string, unknown>, getCategoryTitle(category));
      }
